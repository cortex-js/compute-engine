...
- **RULED 2026-09-03 (Arno): add a boxing validation seam for canonical-handler heads, its own measured round. FIXED 2026-09-04, committed f95efa78 with the duplicated numeric validation removed (Arno: `checkNumericCollections` inside `validateArguments`, `inferNumericArgs` for the numeric context, the handler-side `checkNumericArgs` dropped), measured as follows.** The seam (`applyOperatorDefinition`, `boxed-expression/box.ts`): a `canonical` handler's same-head, still-valid result is validated against the declared signature with the same `validateArguments` call and options the plain path makes, run in check-only mode so it is a GATE — a provably off-carrier operand becomes the error operand a plain head mints, a symbol whose held value lies outside a ranged carrier is refused, and the inference the plain path would make on a fresh symbol is not made (a fresh `x` in `Sin(x)` keeps the `number` the handler's own numeric check gives it). The pure-numeric heads keep the old `checkNumericArgs` re-check as a second gate for a collection of provably non-numeric cells. Two strengths were measured on the 49 most exposed suites (5773 tests): FULL PARITY, inference included, failed 37 tests in 11 suites — the largest class being the trig heads' declared `complex` retyping every fresh `x`, which the compile pins forbid — and the GATE, 19; of those 19, 15 were handlers documented as more lenient than their declarations, closed by widening four signatures (`Apply` takes a function literal, `Approx` a single operand, `At` a base of any type, `DMS` a complex component), 1 was a pin on the old constructor signatures, and 3 were the out-of-window fallback validating without the frame. The distribution constructors declare their carriers as ranged types (`real<0<..>`, `integer<0..>`), so `PoissonDistribution(s)` with `s := -3` is refused at boxing with `incompatible-type`; a literal keeps the handler's `out-of-range`. DISCOVERED, NOT FIXED: `validateArguments` admits a collection with provably non-numeric cells for a threadable numeric parameter on the PLAIN path too — `Ln(["a", "b"])` and `Sqrt(["a", "b"])` are valid — which the numeric re-check masked for the canonical-handler heads; the element-type read belongs in the threadable admission of `validate.ts`, its own measured change. The dual review then widened `Apply`'s callee to `any` (the constant-nullary and expression shorthands `Apply(3, 5)`, `Apply(x + 1, 2)`) and every relation of the family sharing the one-operand chain (`Less(3)` is `True`) to `any*`, gave `DMS` the `number` result its type handler can claim, and moved the timing deviation the trig family and `Power` documented: a provable off-carrier operand such as `Sin(+oo)` is now refused at boxing, and the `type-handler-audit` pin that typed it `number` moved; `Power(x, ~oo)` stays admitted, because `~oo` types `number`. The validation runs in a new CHECK-ONLY mode of `validateArguments` (`noInference`): every verdict, repair and substitution of the plain path, none of its inference on a valueless symbol. A rollback frame around a plain validation was tried first and refuted twice — a repair requested inside the frame was rolled back and requested again by the enclosing rebuild loop (`\sum_{n \in N} K` threw), and the repair-free trial mode that avoids that also switches off the overlap admissions the plain path makes (47 failures). The full suite then surfaced the last declarations narrower than their handlers: the set relations and constructors (`Element`, `NotElement`, the `Subset`/`Superset` family, `Intersection`, `Union`) declare `any` operands, because the geometry set-label tolerance keeps a label operand inert by ruling and the handlers decide a provably wrong one themselves; `Apply`'s arguments are `any*`, because an argument that only evaluates to `Nothing` is bound, never erased. Two pin classes moved: a range as a range bound is now refused at boxing (the compiler's own bound diagnostic is pre-empted), and `Map(Less, …)`/`Filter(…, Less)` are valid at boxing now that a relation accepts one operand, so the compiler's callback gate, not the static arity gate, refuses them. Cost, measured A/B against the commit before the seam in both run orders (four runs each, medians, 4 000 operations per run after warm-up, `seam-bench.ts`): parsing within noise over eight LaTeX inputs; boxing +4.4 % over ten shapes at the commit, concentrated on the heads whose handler does almost nothing — `PoissonDistribution(lam)` 1.9 → 3.4 µs, `Element(x, S)` 2.3 → 3.2 µs, `Sin(x + 1)` +13 %, `Divide(Sin(a), Cos(b))` +14 % — because the cost is the `validateArguments` call itself, about 1.2–1.5 µs per validated node, not the duplicated checks (removing those recovered a third of a microsecond). A profile split the cost in two: for a compound operand it is the operand's TYPE being derived at boxing instead of on first read (HEAD's boxing plus one type read equals the seam's boxing), which no cheaper bound can replace without changing verdicts (`Sin(Ln(0))` is refused only because `Ln(0)` types as an infinity); for a trivial operand it is the machinery on a call that could not be refused. The seam therefore skips the validation for such a call (`triviallyAdmittedCall`, `box.ts`): a single-arm signature without type variables, a fitting arity, and every operand a valueless non-operator symbol whose type overlaps its parameter (the plain path's own rule for a symbolic operand — a `number`-typed `lam` passes `real<0<..>` by overlap) or a number literal whose value type is a subtype of its parameter. Re-measured on an idle box, twice: boxing +2.2 to +2.6 % overall, `PoissonDistribution(lam)` 2.1–2.6 µs against 3.4 at the commit and 1.9 before the seam, `Element(x, S)` +5 to +7 %, the nested quotient +4 %, `Sin(x + 1)` +11 to +13 % unchanged; parsing unchanged; every verdict pinned. The symbolic test is the plain path's: a subtype fits outright, a parameter the runtime conformance check exempts (a callable, a collection kind, `any`, `unknown`, a type variable) is never admitted by overlap, and `typesOverlap` — not the weaker `provablyDisjoint` — decides the rest; a mandatory variadic tail counts in the minimum arity. Pins in `test/compute-engine/boxing-seam-canonical-heads.test.ts`. A distribution built with a SYMBOL parameter and left unevaluated
  ...
  **FOLLOW-UP 2026-09-04 (performance):** the numeric second gate described
  above is removed. The seam's threadable admission now rejects a collection
  whose element carrier is provably non-numeric, and calls the validation-free
  `inferNumericArgs` helper only to preserve the historical numeric inference
  on fresh operands. The ordinary broadcast path remains fail-open so it can
  report failures per cell with their broadcast context. The hot arithmetic
  heads already bypass the seam through `makeNumericFunction`, so
  `Add`/`Multiply`/`Divide`/`Power` keep their single `checkNumericArgs` pass;
  the trig factory leaves strict arity validation to the seam while retaining
  its required non-strict padding.
- **RULED 2026-09-03 (Arno): guard the symbolic forms (`Which` on integrality, `Floor` in the CDF); the compile-parity pin moves. FIXED 2026-09-04 (unstaged, awaiting release): `discreteSupportGuard`/`guardedMass`/`discreteCDFGuard` in `library/distributions.ts` wrap the Poisson and binomial closed forms at a symbolic point in the support tests the literal route applies — `Which(⌊x⌋ = x ∧ x ≥ 0 [∧ x ≤ n], closed, True, 0)` for a pmf, `Which(x < 0, 0, [x ≥ n, 1,] True, closed(⌊x⌋))` for a CDF — dropping each clause the point's type proves (`m: integer<0..>` keeps the bare form). The held arm is evaluated in the caller's mode. The parity pin in `a1-c1-compile-parity.test.ts` now expects the `Which` and checks non-integer and negative points too; route-agreement pins in `error-model-distributions.test.ts`.** The discrete pmfs at a SYMBOLIC point are not
...
- **RULED 2026-09-03 (Arno): add the notation `\|v\|_p` with `_1`, `_2`, `_\infty`, `_F`, accepted on `\|…\|` and `\left\Vert…\right\Vert`; no subscript stays the 2-norm. FIXED 2026-09-04 (unstaged, awaiting release): the `Norm` serializer appends the order subscript (`serializeNormOrder`, `latex-syntax/dictionary/definitions-arithmetic.ts`; order 2 and no order stay bare), and a parse rewrite `parseNormOrder` in `latex-syntax/parse.ts` turns the `Subscript(Norm(v), p)` every norm spelling produces — `\|…\|`, `\left\Vert…\right\Vert`, `\lVert…\rVert`, `||…||`, the `Vmatrix` environment — into `Norm(v, p)`, `F` becoming the `"Frobenius"` order the handler reads. A subscript of several scripts is left as written. Orders 1, ∞, F and a plain number round-trip `.isSame`-exactly; order 2 serializes bare by the ruling, so its round trip is the one-operand norm. Pins in `test/compute-engine/latex-syntax/matchfix.test.ts`.** `Norm(v, p)` does not round-trip through LaTeX today. The serializer
...
- **RULED 2026-09-03 (Arno): fix the serializer. FIXED 2026-09-04 (unstaged, awaiting release): `namedElement` in `common/type/serialize.ts` takes the modifier's precedence and parenthesizes a union, intersection or signature under `?`, `*` and `+`; six pins moved to the parenthesized spelling. ALSO FIXED in the review round: `getPrecedence` had no case for the `numeric`, `expression`, `symbol` and `reference` kinds, so they fell to the lowest precedence and were parenthesized in every nested context — `real<0..> | string` printed `(real<0..>) | string`. They now take the atom precedence. Because a union sorts its members by their printed form, dropping the parenthesis also changes the member ORDER: `(real<0..>) | nan` is now `nan | real<0..>`. Ten test files and thirteen comment lines moved to the bare spelling; the generated `src/epsil/docs/library.md` is regenerated with them.** The type serializer drops the parentheses around a variadic union.
