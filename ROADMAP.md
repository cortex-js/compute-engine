...
### Tycho item 182 — canonicalization-time collection-facet probe storm on a range-broadcast `At` head (OPEN, root-caused 2026-08-14)

Original filing: parsing `255(0.5 + L[1+3(0..Length(D)-1)]/60)/255` inside
a live Tycho document scope burns ~9.4 s in one 5 s span; fresh-engine
variants were ≤21 ms. **Root-caused same day; the early hypotheses in the
filing are corrected by measurement:**

- NOT the repair machinery: instrumented `_withRepairFrame` during the
  real document run — zero rebuilds, zero devolves, max frame depth 4.
- THE MEASURED DISEASE: `countOrUndefinedOnIterationLimit`
  (collection-utils.ts, the synthesized `isEmpty`/`isFinite` defaults) is
  84% of the whole document-open — **210,484 `Range` count probes across
  64,298 distinct Range instances**, reached from canonicalization-time
  facet reads: `checkNumericArgs` → `isFiniteIndexedCollection` →
  `.isFiniteCollection` (validate.ts:459) and `addType` →
  `isFixedShapeCollection`. Each probe is cheap (~0.1 ms); the count of
  probes — repeated per arithmetic ancestor, per fresh instance, with no
  facet memo — is the blowup. Same family as the shape-query
  double-read = 2^depth class and item 181's probe cascades, but through
  the UNCACHED collection facets, so the 181 fix does not touch it.
- **Severity upgrade: without a deadline it is an OOM crash, not a
  hang.** Bare CE-source repro (no Tycho code): fresh engine + the
  document's real `L` (a 19k-element literal) + user fns + `D` assigned
  the UNEVALUATED lazy comprehension → parsing ONE slot of the head
  (`\frac{255(0.5+\frac{1}{60}L[1+3(0..(\mathrm{Length}(D)-1))])}{255}`)
  exhausts a 4 GB heap in ~130 s (`Length(D)` itself evaluates in 3 ms,
  to a SYMBOLIC Length — the storm is in the facet probes and whatever
  they materialize, not in resolving D). Tycho's 5 s guard is currently
  load-bearing against a crash class.
- Repro, all in-repo: `docs/scratch/2026-08-14-item182-bare-repro.mts`
  (no arg = the OOM arm; `5000` arg = deadlined ~5 s salvage; prereq
  `/tmp/lizeq-cells.json`, regenerated by
  `dev/tycho/docs/scratch/d21-dump-cells.mts`). End-to-end validation
  against the real document:
  `docs/scratch/2026-08-14-item182-ce-src-redirect.mjs` runs Tycho's
  `d21-lizeq-head-extract.mts` harness on CE SOURCE (span#2 reproduces
  at ~10.4 s) — usage in the file header; no Tycho-side run needed.
  Observed while verifying the deadlined arm: an `Assertion failed`
  console line during the deadline salvage — un-triaged.

Open fix questions (next session): why one canonicalization emits 64K
fresh Range instances (broadcast/flatten churn over the 19k literal?);
what the OOM arm materializes; and where the memo belongs — per-instance
facet caching on `isFiniteCollection`/count (mind the lazy-collection
evaluate-memo design's refuted variants,
`docs/plans/2026-08-09-lazy-collection-evaluate-design.md`) vs failing
open (skip the probe) on symbolic-bound Ranges in
`checkNumericArgs`/`addType`. Second observation from the filing stands:
