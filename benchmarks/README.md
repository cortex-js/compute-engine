...
( cd benchmarks/.competitors && npm pack @cortex-js/compute-engine@0.92.1 \
  && mkdir -p ce-0.92.1 && tar xzf cortex-js-compute-engine-0.92.1.tgz -C ce-0.92.1 --strip-components=1 )
...
| `CE_PUBLISHED_VERSION` | `0.92.1` | Label for the published column. |
...
- `ops-bench.mjs` (CE bundle) + `ops-bench.py` (mpmath) + `run-ops.mjs` (driver)
  ...
  vs published 0.92.1 vs mpmath. Where the transcendental tables measure whole
