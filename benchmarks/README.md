...
( cd benchmarks/.competitors && npm pack @cortex-js/compute-engine@0.99.0 \
  && mkdir -p ce-0.99.0 && tar xzf cortex-js-compute-engine-0.99.0.tgz -C ce-0.99.0 --strip-components=1 )
...
| `CE_PUBLISHED_VERSION` | `0.99.0` | Label for the published column. |
...
- `ops-bench.mjs` (CE bundle) + `ops-bench.py` (mpmath) + `run-ops.mjs` (driver)
  ...
  vs published 0.99.0 vs mpmath. Where the transcendental tables measure whole
