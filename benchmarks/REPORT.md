...
|---|---|---|
| Compute Engine — current build | `0.128.10` @ `bf918be0` (freshly built from `src/`) | Node v22.13.1 |
...
| Compute Engine — published | `0.128.9` (npm) | Node v22.13.1 |
...
- **Columns**: the current build and published `0.128.9` are compared as base engines; a third CE column (`CE+R/F`) is the current build with the experimental **Rubi** integrator and **Fungrim** identities enabled. SymPy, math.js, NumPy and Wolfram are the competitors.
...
| Category | CE·cur | CE+R/F | CE·0.128.9 | SymPy | math.js | NumPy | Wolfram |
|---|---|---|---|---|---|---|---|
...
> All three CE columns (`CE·cur`, `CE·0.128.9`, `CE+R/F`) are measured **warm, in one shared process**, so they are directly comparable to each other in every row. `CE+R/F` (current minified bundle + the opt-in Rubi + Fungrim rule packs, loaded once via `loadIntegrationRules` / `loadIdentities`) **tries matching ~2,647 Rubi rules** before falling back to the built-in integrator — so its integral times include that match attempt even when no rule applies (e.g. `∫xeˣ`); on rows where no rule can fire (numeric, differentiation) `CE·cur` and `CE+R/F` should read ≈equal. The honest per-op pack overhead is tabulated in the [Rule packs](#rule-packs--coverage--true-warm-overhead) section.

### Arbitrary-precision numeric evaluation — times in **µs**

| # | Case | CE·cur | CE+R/F | CE·0.128.9 | SymPy | math.js | NumPy | Wolfram |
|---|---|---|---|---|---|---|---|---|
| | **Core tier** |  |  |  |  |  |  |  |
| N01 | $\pi^2$ <sub>(50d)</sub> | 13 | 13 | 15 | 178 | 48 | 🟡 <sub>16 digits</sub> 3.7 | 3.8 |
| N02 | $e$ <sub>(50d)</sub> | 0.58 | 0.58 | 0.62 | 157 | 11 | 🟡 <sub>16 digits</sub> 3.1 | 2.9 |
| N03 | $\sqrt2$ <sub>(50d)</sub> | 6.2 | 15 | 7.5 | 232 | 87 | 🟡 <sub>17 digits</sub> 5.2 | 4.4 |
| N04 | $100!$ <sub>(exact)</sub> | 13 | 13 | 13 | 264 | 108 | ❌ <sub>inexact</sub> 11 | 2.6 |
| N05 | $e^{\pi}$ <sub>(40d)</sub> | 13 | 13 | 15 | 194 | 363 | 🟡 <sub>17 digits</sub> 4.9 | 3.9 |
| | **Hard tier** |  |  |  |  |  |  |  |
| N06 | $\pi$ <sub>(200d)</sub> | 0.54 | 0.54 | 0.58 | 155 | 12 | 🟡 <sub>16 digits</sub> 3.2 | 2.9 |
| N07 | $\zeta(3)$ <sub>(40d)</sub> | 286 | 248 | 336 | 263 | ❌ <sub>8 digits</sub> 3616 | — | 22 |
| N08 | $\Gamma(\tfrac13)$ <sub>(40d)</sub> | 167 | 156 | 178 | 274 | ⚠️ | — | 45 |
| N09 | $W(1)$ <sub>(40d)</sub> | 58 | 55 | 69 | 670 | — | — | 38 |
|  | **median µs** | **13** | **13** | **15** | **232** | **87** | **4.9** | **3.9** |

### Simplification

| # | Case | CE·cur | CE+R/F | CE·0.128.9 | SymPy | math.js | Wolfram |
|---|---|---|---|---|---|---|---|
| | **Core tier** |  |  |  |  |  |  |
| S01 | $\frac{x^2-1}{x-1}$ | 0.16 | 0.17 | 0.27 | 8.68 | 🟡 <sub>not simplified</sub> 1.31 | 0.17 |
| S02 | $\sin^2 x+\cos^2 x$ | 0.10 | 0.20 | 0.15 | 8.87 | 🟡 <sub>not simplified</sub> 1.01 | 0.08 |
| S03 | $(x+1)^2-(x-1)^2$ | 0.32 | 0.46 | 0.51 | 6.19 | 🟡 <sub>not simplified</sub> 1.00 | 0.15 |
| S04 | $\frac{x^3-x}{x}$ | 0.21 | 0.16 | 0.26 | 4.44 | 1.52 | 0.63 |
| S05 | $x^{-1/2}-\frac{1}{\sqrt x}$ | 0.10 | 0.11 | 0.18 | 0.25 | 🟡 <sub>not simplified</sub> 1.63 | 0.02 |
| | **Hard tier** |  |  |  |  |  |  |
| S06 | $\sqrt6\,x+\sqrt2\,x$ | 0.33 | 0.35 | 0.52 | 5.88 | 1.14 | 18.9 |
| S07 | $\ln x+\ln(x+1)$ | 0.22 | 0.38 | 0.34 | 7.08 | 🟡 <sub>not simplified</sub> 0.99 | 1.41 |
| S08 | $\sqrt{3+2\sqrt2}$ | 0.09 | 0.12 | 0.14 | 🟡 <sub>not simplified</sub> 3.54 | 🟡 <sub>numeric only</sub> 0.96 | 3.35 |
| S09 | $\frac{x^3-1}{x-1}$ | 0.13 | 0.14 | 0.22 | 9.19 | 🟡 <sub>not simplified</sub> 1.10 | 1.02 |
|  | **median ms** | **0.16** | **0.17** | **0.26** | **6.19** | **1.10** | **0.63** |
...
|---|---|---|---|---|---|---|---|
| | **Core tier** |  |  |  |  |  |  |
| D01 | $\tfrac{d}{dx}\sin x$ | 0.03 | 0.03 | 0.04 | 0.33 | 0.69 | 0.0035 |
| D02 | $\tfrac{d}{dx}x^5$ | 0.11 | 0.11 | 0.17 | 0.50 | 1.20 | 0.0038 |
| D03 | $\tfrac{d}{dx}\tan x$ | 0.06 | 0.06 | 0.09 | 2.19 | 0.89 | 0.0036 |
| D04 | $\tfrac{d}{dx}x^2\sin x$ | 0.29 | 0.29 | 0.39 | 2.05 | 1.88 | 0.0055 |
| D05 | $\tfrac{d}{dx}\sin(x^2)$ | 0.14 | 0.14 | 0.21 | 1.43 | 1.34 | 0.0045 |
| | **Hard tier** |  |  |  |  |  |  |
| D06 | $\tfrac{d}{dx}x^x$ | 0.12 | 0.11 | 0.14 | 1.82 | 1.76 | 0.005 |
| D07 | $\tfrac{d}{dx}\arcsin x$ | 0.19 | 0.18 | 0.26 | 2.98 | 1.13 | 0.0046 |
| D08 | $\tfrac{d}{dx}\ln(\sin x)$ | 0.07 | 0.07 | 0.10 | 1.12 | 1.06 | 0.0043 |
| D09 | $\tfrac{d}{dx}\sqrt{1-x^2}$ | 0.33 | 0.35 | 0.41 | 6.51 | 2.20 | 0.0079 |
|  | **median ms** | **0.12** | **0.11** | **0.17** | **1.82** | **1.20** | **0.0045** |

### Antiderivation (symbolic integration)

| # | Case | CE·cur | CE+R/F | CE·0.128.9 | SymPy | Wolfram |
|---|---|---|---|---|---|---|
| | **Core tier** |  |  |  |  |  |
| A01 | $\int x^2\,dx$ | 0.16 | 0.18 | 0.22 | 0.37 | 0.03 |
| A02 | $\int\sin x\,dx$ | 0.05 | 0.25 | 0.08 | 1.23 | 0.61 |
| A03 | $\int x e^x\,dx$ | 0.21 | 1.10 | 0.26 | 6.98 | 0.59 |
| A04 | $\int\frac{1}{1+x^2}\,dx$ | 0.12 | 0.27 | 0.17 | 9.54 | 0.89 |
| A05 | $\int\frac{x}{x^2+1}\,dx$ | 0.32 | 1.34 | 0.53 | 7.26 | 0.62 |
| | **Hard tier** |  |  |  |  |  |
| A06 | $\int\frac{1}{x^3+1}\,dx$ | 2.64 | 14.4 | 3.40 | 25.4 | 8.29 |
| A07 | $\int\frac{1}{\sqrt x}\,dx$ | 0.14 | 0.22 | 0.17 | 0.72 | 0.37 |
| A08 | $\int e^{-x^2}\,dx$ | 0.31 | 0.85 | 0.37 | 26.7 | 0.46 |
| A09 | $\int\frac{x}{\sqrt{1-x^2}}\,dx$ | 0.37 | 2.16 | 0.50 | 24.8 | 2.14 |
| CR1 | $\int\frac{\sqrt x}{1+x}\,dx$ | **∅** | 1.82 | ∅ | 21.7 | 2.23 |
| CR2 | $\int\frac{x}{(1+x)^{1/3}}\,dx$ | **∅** | 1.61 | ∅ | 122 | 1.14 |
| CR3 | $\int\frac{x^2}{(1+x)^{1/3}}\,dx$ | **∅** | 2.06 | ∅ | 212 | 1.50 |
|  | **median ms** | **0.21** | **1.34** | **0.26** | **21.7** | **0.89** |
...
|---|---|---|---|---|
| A09 | $\int\frac{x}{\sqrt{1-x^2}}\,dx$ | 366 | 2157 | 5.89× |
| A06 | $\int\frac{1}{x^3+1}\,dx$ | 2642 | 14424 | 5.46× |
| A03 | $\int x e^x\,dx$ | 208 | 1104 | 5.32× |
| A02 | $\int\sin x\,dx$ | 50 | 254 | 5.11× |
| CR1 | $\int\frac{\sqrt x}{1+x}\,dx$ | 407 | 1820 | 4.47× |
| A05 | $\int\frac{x}{x^2+1}\,dx$ | 324 | 1342 | 4.15× |
| CR3 | $\int\frac{x^2}{(1+x)^{1/3}}\,dx$ | 536 | 2063 | 3.85× |
| CR2 | $\int\frac{x}{(1+x)^{1/3}}\,dx$ | 470 | 1607 | 3.42× |
| A08 | $\int e^{-x^2}\,dx$ | 311 | 845 | 2.72× |
| CE4 | $\int_{-\infty}^{\infty} e^{-x^2}\,dx$ | 332 | 820 | 2.47× |
| N03 | $\sqrt2$ | 6.2 | 15 | 2.40× |
| CE1 | $\lim_{x\to0}\tfrac{\sin x}{x}$ | 71 | 163 | 2.27× |
| A04 | $\int\frac{1}{1+x^2}\,dx$ | 119 | 270 | 2.27× |
| S02 | $\sin^2 x+\cos^2 x$ | 99 | 203 | 2.04× |
| S07 | $\ln x+\ln(x+1)$ | 223 | 378 | 1.70× |
| A07 | $\int\frac{1}{\sqrt x}\,dx$ | 138 | 217 | 1.57× |
| S03 | $(x+1)^2-(x-1)^2$ | 322 | 456 | 1.41× |
| S08 | $\sqrt{3+2\sqrt2}$ | 91 | 119 | 1.31× |
| D01 | $\tfrac{d}{dx}\sin x$ | 31 | 34 | 1.11× |
| CE2 | $\lim_{x\to\infty}(1+\tfrac1x)^x$ | 1781 | 1960 | 1.10× |
| A01 | $\int x^2\,dx$ | 165 | 177 | 1.08× |
| N07 | $\zeta(3)$ | 286 | 248 | **0.87× (win)** |
| CE3 | $\int_1^2\tfrac1x\,dx$ | 112 | 93 | **0.83× (win)** |
| S04 | $\frac{x^3-x}{x}$ | 212 | 158 | **0.74× (win)** |

_Times in µs (warm median). 29 row(s) within ±10% (no measurable pack overhead — numeric / differentiation) omitted._

## Current build vs published `0.128.9`

No behavioural differences detected on this suite — the current build matches `0.128.9` on all 39 cases (correctness and output form).
