(* Extracted from https://math.unm.edu/~wester/demos/Statistics/Math.problems
...
(* Compute the first quartile (25% quantile) of a list of numbers => 
2 or 5/2 *)
...
(* Compute the mode (the most frequent item) of  a list of numbers 
=> 7 *)

Mode[{3, 7, 11, 7, 3, 5, 7}]

(* Compute the unbiased sample standard deviation of a list of 
numbers
   => sqrt(5/2) *)
...
(* Discrete distributions---what is the probability of finding 
exactly 12
   switches that work from a group of 15 where the probability of a single 
one
   working is 75%?  (Need to use the probability density function [PDF] of 
the
   binomial distribution.) => 0.22520 *)

PDF[BinomialDistribution[15, .75], 12]

(* Replace `exactly' by `up through' in the above.  (Need to use the 
cumulative
   probability density function [CDF] of the binomial distribution.) => 
0.76391
   *)
...
(* Continuous distributions---if a radiation emission can be modeled 
by a
   normal distribution with a mean of 4.35 mrem and a standard deviation of
   0.59 mrem, what is the probability of being exposed to anywhere from 4 to 
5
   mrem? => .5867 *)
...
MeanTest[xx, 5, KnownStandardDeviation -> StandardDeviation[xx]]

Clear[xx]

(* Chi-square test---what is the expectation that row 
characteristics are
...
rowSum = Table[Sum[x[[i, j]], {j, 1, n}], {i, 1, m}];

colSum = Table[Sum[x[[i, j]], {i, 1, m}], {j, 1, n}];

matSum = Sum[rowSum[[i]], {i, 1, m}];

e = Table[rowSum[[i]]*colSum[[j]]/matSum, {i, 1, m}, {j, 1, n}];

chi2 = Sum[(x[[i, j]] - e[[i, j]])^2/e[[i, j]], {i, 1, m}, {j, 1, n}]
...
Clear[chi2, colSum, matSum, rowSum, e, m, n, x]

<< Statistics`LinearRegression`

(* Linear regression (age as a function of developmental score).  
See Lambert
   H. Koopmans, _Introduction to Contemporary Statistical Methods_, Second
   Edition, Duxbury Press, 1987, p. 459 => y' = 0.7365 x + 6.964 *)

t = {{3.33, 3.25, 3.92, 3.50,  4.33,  4.92,  6.08,  7.42,  8.33,  
8.00,  9.25,
         10.75},
     {8.61, 9.40, 9.86, 9.91, 10.53, 10.61, 10.59, 13.28, 12.76, 13.44, 
14.27,
         14.13}}

Fit[Transpose[%], {1, x}, x]

Regress[Transpose[t], {1, x}, x]

Clear[t]

(* Multiple linear regression (income as a function of age and years 
of
   college) => y = -16278.7 + 960.925 x1 + 2975.66 x2 *)

{{37, 45, 38, 42, 31}, {4, 0, 5, 2, 4}, {31200, 26800, 35000, 30300, 
25400}}

Fit[Transpose[%], {1, x1, x2}, {x1, x2}]

(* Multiple linear regression using the L1 or Least Absolute 
Deviations
   technique rather than the Least Squares technique (minimizing the sum of 
the
   ...
   flow to the plant, cooling water inlet temperature and acid 
concentration).
...
{{80, 80, 75, 62, 62, 62, 62, 62, 58, 58, 58, 58, 58, 58, 50, 50, 
50, 50, 50,
  56, 70},
...
(* Nonlinear regression (Weight Loss Data from an Obese Patient 
consisting of
   ...
   b0 + b1 2^(- days/th), starting at (b0, b1, th) = (90, 95, 120)   
[Venables
   and Ripley, p. 225] => weight = 81.37375 + 102.6842 2^(- days/141.9105) *)

{{  0,   4,   7,   7,  11,  18,  24,  30,  32,  43,  46,  60,  64,  
70,  71,
...
NonlinearFit[Transpose[%], b0 + b1*2^(- days/th), {days}, {b0, b1, 
th}]
