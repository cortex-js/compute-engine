(* Extracted from https://math.unm.edu/~wester/demos/Algebra/Math.problems
...
(* One would think that the simplification 2 2^n => 2^(n + 1) would 
happen
...
(* Univariate gcd: gcd(p1, p2) => 1, gcd(p1 q, p2 q) => q   [Richard 
Liska] *)
...
PolynomialGCD[Expand[p1*q], Expand[p2*q]] - q
...
PolynomialGCD[2*x^(n + 4) - x^(n + 2), 4*x^(n + 1) + 3*x^n]

(* Resultants.  If the resultant of two polynomials is zero, this 
implies they
   have a common factor.  See Keith O. Geddes, Stephen R. Czapor and George
   Labahn, _Algorithms for Computer Algebra_, Kluwer Academic Publishers, 
1992,
   p. 286 => 0 *)

Resultant[3*x^4 + 3*x^3 + x^2 - x - 2, x^3 - 3*x^2 + x + 5, x]

(* Numbers are nice, but symbols allow for variability---try some 
high school
...
(* Completely factor this polynomial, then try to multiply it back 
together! *)

Solve[x^3 + x^2 - 7 == 0]

Apply[Times, Map[#[[1]] - #[[2]] &, Flatten[%]]]
...
Factor[4*x^4 + 8*x^3 + 77*x^2 + 18*x + 153, Extension -> I]
...
(* => (x^2 + x + 1) (x^9 - x^8 + x^6 - x^5 + x^3 - x^2 + 1)  mod  
65537
      [Paul Zimmermann] *)
...
q = Expand[(1 - c^2)^5 * (1 - s^2)^5 * (c^2 + s^2)^10];

GroebnerBasis[{q, c^2 + s^2 - 1}, {c, s}]
...
Factor[4*x^2 - 21*x*y + 20*y^2, Modulus -> 3]

(* => 1/4 (x + y) (2 x +  y [-1 + i sqrt(3)]) (2 x + y [-1 - i 
sqrt(3)]) *)

Factor[x^3 + y^3, Extension -> I*Sqrt[3]]

(* Partial fraction decomposition => 3/(x + 2) - 2/(x + 1) + 2/(x + 
1)^2 *)
...
(* Noncommutative algebra: note that (A B C)^(-1) = C^(-1) B^(-1) 
A^(-1)
...
comm3[A_, B_, C_]:= comm2[A, comm2[B, C]];
