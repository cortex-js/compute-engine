...
_A quick, like-for-like comparison of Compute Engine against widely-used open-source math libraries. 2026-08-02._
...
|---|---|--:|--:|--:|--:|
| Evaluate | $\pi$ <sub>(200 digits)</sub> | <0.01 ms | 0.17 ms | 0.01 ms | <0.01 ms |
| Evaluate | $100!$ <sub>(exact)</sub> | <0.01 ms | 0.28 ms | 0.12 ms | <0.01 ms |
| Evaluate | $\Gamma(\tfrac13)$ <sub>(40 digits)</sub> | 0.17 ms | 0.27 ms | — | 0.05 ms |
| Simplify | $\frac{x^2-1}{x-1}$ | 0.15 ms | 8.7 ms | 1.6 ms | 0.18 ms |
| Simplify | $\sin^2 x+\cos^2 x$ | 0.08 ms | 9.2 ms | 0.81 ms | 0.08 ms |
| Simplify | $\frac{x^3-1}{x-1}$ | 0.12 ms | 9.9 ms | 1.0 ms | 1.1 ms |
| Differentiate | $\tfrac{d}{dx}x^x$ | 0.10 ms | 2.0 ms | 2.6 ms | <0.01 ms |
| Differentiate | $\tfrac{d}{dx}x^2\sin x$ | 0.22 ms | 2.5 ms | 2.8 ms | <0.01 ms |
| Integrate | $\int x e^x\,dx$ | 0.22 ms | 7.9 ms | — | 0.60 ms |
| Integrate | $\int\frac{1}{1+x^2}\,dx$ | 0.10 ms | 10.9 ms | — | 0.89 ms |
| Integrate | $\int\frac{1}{x^3+1}\,dx$ | 2.6 ms | 26.7 ms | — | 8.5 ms |

On the symbolic operations shared with SymPy above, **Compute Engine is roughly 62× faster per call** (median 0.15 ms vs 9.2 ms) — while running in the browser rather than requiring a Python backend.
...
_Versions: Compute Engine 0.99.0, SymPy 1.14.0, math.js 15.2.0, NumPy 2.4.2, Mathematica 14.3.0 for Mac OS X ARM. Methodology and the full case list: [REPORT.md](./REPORT.md). Reproduce: `node benchmarks/report.mjs && node benchmarks/report_marketing.mjs`._
