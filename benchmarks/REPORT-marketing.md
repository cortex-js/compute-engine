...
_A quick, like-for-like comparison of Compute Engine against widely-used open-source math libraries. 2026-09-23._
...
|---|---|--:|--:|--:|--:|
| Evaluate | $\pi$ <sub>(200 digits)</sub> | <0.01 ms | 0.09 ms | <0.01 ms | <0.01 ms |
| Evaluate | $100!$ <sub>(exact)</sub> | <0.01 ms | 0.16 ms | 0.06 ms | <0.01 ms |
| Evaluate | $\Gamma(\tfrac13)$ <sub>(40 digits)</sub> | 0.10 ms | 0.15 ms | — | 0.03 ms |
| Simplify | $\frac{x^2-1}{x-1}$ | 0.10 ms | 5.6 ms | 0.52 ms | 0.11 ms |
| Simplify | $\sin^2 x+\cos^2 x$ | 0.06 ms | 5.9 ms | 0.41 ms | 0.05 ms |
| Simplify | $\frac{x^3-1}{x-1}$ | 0.10 ms | 6.1 ms | 0.52 ms | 0.67 ms |
| Differentiate | $\tfrac{d}{dx}x^x$ | 0.07 ms | 1.2 ms | 1.0 ms | <0.01 ms |
| Differentiate | $\tfrac{d}{dx}x^2\sin x$ | 0.18 ms | 1.4 ms | 1.1 ms | <0.01 ms |
| Integrate | $\int x e^x\,dx$ | 0.12 ms | 4.6 ms | — | 0.42 ms |
| Integrate | $\int\frac{1}{1+x^2}\,dx$ | 0.07 ms | 6.5 ms | — | 0.64 ms |
| Integrate | $\int\frac{1}{x^3+1}\,dx$ | 1.4 ms | 16.5 ms | — | 5.8 ms |

On the symbolic operations shared with SymPy above, **Compute Engine is roughly 58× faster per call** (median 0.10 ms vs 5.9 ms) — while running in the browser rather than requiring a Python backend.
...
_Versions: Compute Engine 0.133.0, SymPy 1.14.0, math.js 15.2.0, NumPy 2.4.2, Mathematica 15.0.0 for Mac OS X ARM. Methodology and the full case list: [REPORT.md](./REPORT.md). Reproduce: `node benchmarks/report.mjs && node benchmarks/report_marketing.mjs`._
