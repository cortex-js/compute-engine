...
for cid, title, ce, sy, f in [
    ("F1", "x² − 1", ["Subtract", ["Power", "x", 2], 1], "x**2 - 1", lambda x: x**2 - 1),
    ("F2", "x³ − 1", ["Subtract", ["Power", "x", 3], 1], "x**3 - 1", lambda x: x**3 - 1),
    ("F3", "x⁴ − 1", ["Subtract", ["Power", "x", 4], 1], "x**4 - 1", lambda x: x**4 - 1),
    ("F4", "x⁶ − 1", ["Subtract", ["Power", "x", 6], 1], "x**6 - 1", lambda x: x**6 - 1),
    ("F5", "x⁷ − 1", ["Subtract", ["Power", "x", 7], 1], "x**7 - 1", lambda x: x**7 - 1),
]:
    add(id=cid, cat="factor", title=title,
...
for cid, title, ce, sy, g in [
    ("G1", "gcd((x+1)(x+2), (x+1)(x+3))",
    ...
    add(id=cid, cat="gcd", title=title,
...
for cid, title, ce, sy, f in [
    ("E1", "(x+1)⁵", ["Power", ["Add", "x", 1], 5], "(x + 1)**5", lambda x: (x + 1) ** 5),
    ("E2", "(x+2)⁴", ["Power", ["Add", "x", 2], 4], "(x + 2)**4", lambda x: (x + 2) ** 4),
    ("E3", "(x−1)⁶", ["Power", ["Subtract", "x", 1], 6], "(x - 1)**6", lambda x: (x - 1) ** 6),
]:
    add(id=cid, cat="expand", title=title,
        ce={"op": "expand", "mathjson": ce}, sympy={"op": "expand", "expr": sy},
        verify={"kind": "equiv", "form": "expanded", "points": [s(p) for p in P], "values": samples(f)})

# ---- Simplification ----
for cid, title, ce, sy, f in [
    ("S1", "(x²−1)/(x−1)", ["Divide", ["Subtract", ["Power", "x", 2], 1], ["Subtract", "x", 1]],
     "(x**2 - 1)/(x - 1)", lambda x: (x**2 - 1) / (x - 1)),
    ("S2", "(x³−1)/(x−1)", ["Divide", ["Subtract", ["Power", "x", 3], 1], ["Subtract", "x", 1]],
     "(x**3 - 1)/(x - 1)", lambda x: (x**3 - 1) / (x - 1)),
    ("S3", "x^(−1/2) − 1/√x", ["Subtract", ["Power", "x", ["Divide", -1, 2]], ["Divide", 1, ["Sqrt", "x"]]],
     "x**Rational(-1,2) - 1/sqrt(x)", lambda x: x ** (mpf(-1) / 2) - 1 / sqrt(x)),
]:
    add(id=cid, cat="simplify", title=title,
...
for cid, title, ce, sy, f in [
    ("I1", "∫ x² dx", ["Power", "x", 2], "x**2", lambda x: x**2),
    ("I2", "∫ 1/(1+x²) dx", ["Divide", 1, ["Add", 1, ["Power", "x", 2]]], "1/(1 + x**2)", lambda x: 1 / (1 + x**2)),
    ("I3", "∫ 1/√x dx", ["Divide", 1, ["Sqrt", "x"]], "1/sqrt(x)", lambda x: 1 / sqrt(x)),
    ("I4", "∫ e^(−x²) dx", ["Power", "ExponentialE", ["Negate", ["Power", "x", 2]]], "exp(-x**2)", lambda x: exp(-x**2)),
    ("I5", "∫ 1/(x³+1) dx", ["Divide", 1, ["Add", ["Power", "x", 3], 1]], "1/(x**3 + 1)", lambda x: 1 / (x**3 + 1)),
]:
    add(id=cid, cat="integrate", title=title,
...
# ---- Limits ----
for cid, title, body, point, sy, val in [
    ("L1", "lim_{x→0} sin x / x", ["Divide", ["Sin", "x"], "x"], 0, "sin(x)/x", mpf(1)),
    ("L2", "lim_{x→0} (1−cos x)/x²", ["Divide", ["Subtract", 1, ["Cos", "x"]], ["Power", "x", 2]], 0, "(1 - cos(x))/x**2", mpf(1) / 2),
    ("L3", "lim_{x→0} (x²−1)/(x−1)... @1", ["Divide", ["Subtract", ["Power", "x", 2], 1], ["Subtract", "x", 1]], 1, "(x**2 - 1)/(x - 1)", mpf(2)),
]:
    add(id=cid, cat="limit", title=title,
