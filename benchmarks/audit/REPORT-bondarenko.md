...
- **CE 0/35** · **CE+R/F 21/35** · **SymPy 7/35** · **Mathematica 32/35** correct.
- Against the **Mathematica** baseline, base CE trails on **32** cases; **20** of those recovered by Rubi/Fungrim.

| Operation | CE | CE+R/F | SymPy | Mathematica |
|---|--:|--:|--:|--:|
| Indefinite ∫ | 0/35 | 21/35 | 7/35 | 32/35 |

## Per-case results

| # | Steps | Integrand | CE | CE+R/F | SymPy | Mathematica | CE+R/F result |
|--:|--:|---|:-:|:-:|:-:|:-:|---|
| 1 | 1 | `1 / (sin(z) + cos(z) + sqrt(2))` | ∅ | ✅ | ✅ | ✅ | `-((-sqrt(2) * sin(z) + 1) / (-sin(z) + c` |
...
| 9 | 4 | `1 / (sqrt(x + sqrt(x^2 + 1)) + 1)` | ∅ | ✅ | ∅ | ✅ | `-2ln(sqrt(x + sqrt(x^2 + 1)) + 1) + 1/2 ` |
...
| 13 | 20 | `sqrt(x + sqrt(x + 1)) / ((x^2 + 1) * sqr` | ∅ | ∅ | ∅ | ✅ | `int(sqrt(x + sqrt(x + 1)) / ((x^2 + 1) *` |
...
| 25 | 5 | `sqrt(sinh(2x) / cosh(x))` | ∅ | ✅ | ∅ | ✅ | `(-2e^(-1/2 * x) * e^x * sqrt(-e^(-2x) + ` |
