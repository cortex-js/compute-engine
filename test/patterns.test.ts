...
describe('PATTERNS  MATCH', () => {
  test('Universal wildcard', () => {
    ...
    // Commutative
    expect(match(['Add', 2, 1], pattern)).toMatchInlineSnapshot();
    expect(match(['Add', 2, 1, 3], pattern)).toMatchInlineSnapshot();
    // Associative
  ...
  test('Sequence wildcard', () => {
    expect(match(['Add', 1, 2, 3, 4], ['Add', 1, '__a']))
    ...
    `);
    expect(match(['Add', 1, 2, 3, 4], ['Add', 1, '__a', 4]))
      .toMatchInlineSnapshot(`
      Object {
        "a": Array [
          "Sequence",
