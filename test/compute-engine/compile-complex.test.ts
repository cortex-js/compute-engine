...
describe('COMPILE COMPLEX - a declared `complex` PARAMETER', () => {
  ...
  it('a value-position reference coerces raw elements: Map(Q, real list)', () => {
    ...
    // reaches `Map` through the broadcasting wrapper every scalar-parameter
    // function takes in value position: since the user ruling of 2026-09-22
    // the interpreter maps such a callback over an element that is itself a
    // collection, whether its parameter is a DECLARED scalar such as `complex`
    // or an open one. The wrapper's callee is the shim, so a raw element is
    // still coerced.
