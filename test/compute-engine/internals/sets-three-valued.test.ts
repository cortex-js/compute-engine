...
describe('Interval with an infinite endpoint', () => {
  ...
  it('contains literals below/at/above the (closed) upper boundary', () => {
    const ce = freshEngine();
    const ray = ce.box(NEGATIVE_RAY); // (-∞, 0]
  ...
  it('excludes the open infinite endpoint, includes a closed one', () => {
    const ce = freshEngine();
    const openRay = ce.box(NEGATIVE_RAY); // (-∞, 0]
    expect(openRay.contains(ce.box('NegativeInfinity'))).toBe(false);

    const closedRay = ce.box(['Interval', 'NegativeInfinity', 0]); // [-∞, 0]
    expect(closedRay.contains(ce.box('NegativeInfinity'))).toBe(true);
  });

  it('distinguishes open vs closed finite endpoints', () => {
    const ce = freshEngine();
    const upRay = ce.box(['Interval', ['Open', 0], 'PositiveInfinity']);
    expect(upRay.contains(ce.number(0))).toBe(false); // open: excluded
    expect(upRay.contains(ce.number(0.5))).toBe(true);
    const upRayClosed = ce.box(['Interval', 0, 'PositiveInfinity']);
  ...
  it('decides symbolic members from assumed bounds', () => {
    ...
    // x > 0 refutes x ∈ (-∞, 0]
    expect(ce.box(NEGATIVE_RAY).contains(ce.box('x'))).toBe(false);
    ce.popScope();
  });
});
...
describe('SetMinus(CC, Interval(Open(-oo), 0)) round trip (gamma/37a95a shape)', () => {
  it('assumes and verifies the membership', () => {
    const ce = freshEngine();
    ce.pushScope();
    expect(ce.assume(ce.box(['Element', 'z', CC_MINUS_RAY]))).toBe('ok');
    expect(ce.verify(ce.box(['Element', 'z', CC_MINUS_RAY]))).toBe(true);
    // The decomposed exclusion verifies too
    expect(ce.verify(ce.box(['NotElement', 'z', NEGATIVE_RAY]))).toBe(true);
    ce.popScope();
  });

  it('is undefined without the assumption (not false)', () => {
    const ce = freshEngine();
    expect(ce.verify(ce.box(['Element', 'z', CC_MINUS_RAY]))).toBeUndefined();
  });

  it('decides concrete members of the SetMinus', () => {
    const ce = freshEngine();
    const cut = ce.box(CC_MINUS_RAY);
    expect(cut.contains(ce.number(1))).toBe(true); // in ℂ, not on the cut
    expect(cut.contains(ce.box('ImaginaryUnit'))).toBe(true);
  ...
  it('the direct NotElement conjunct still discharges (regression)', () => {
    const ce = freshEngine();
    ce.pushScope();
    expect(ce.assume(ce.box(['Element', 'z', 'ComplexNumbers']))).toBe('ok');
    expect(
      ce.assume(
        ce.box(['NotElement', 'z', NEGATIVE_RAY], { canonical: false })
      )
    ).toBe('ok');
    expect(ce.verify(ce.box(['NotElement', 'z', NEGATIVE_RAY]))).toBe(true);
    ce.popScope();
...
describe('finite Set contains', () => {
  it('is undefined for a symbolic element, decided for literals', () => {
    const ce = freshEngine();
    const set = ce.box(['Set', -1, 1]);
    expect(set.contains(ce.box('omega'))).toBeUndefined();
  ...
  it('discharges a stored membership fact (coulomb_wave/01af55 shape)', () => {
    const ce = freshEngine();
    ce.pushScope();
    expect(ce.assume(ce.box(['Element', 'omega', ['Set', -1, 1]]))).toBe('ok');
    expect(ce.verify(ce.box(['Element', 'omega', ['Set', -1, 1]]))).toBe(true);
    ce.popScope();
    // ...and is undefined again outside the scope
    expect(
      ce.verify(ce.box(['Element', 'omega', ['Set', -1, 1]]))
    ).toBeUndefined();
  });

  it('matches symbolic elements structurally', () => {
    const ce = freshEngine();
    const set = ce.box(['Set', 'a', 'b']);
    expect(set.contains(ce.box('a'))).toBe(true);
    expect(set.contains(ce.box('c'))).toBeUndefined(); // could equal a or b
  });
});
...
describe('Range contains', () => {
  it('is undefined for a symbolic element, decided for literals', () => {
    const ce = freshEngine();
    const range = ce.box(['Range', 1, 5]);
    expect(range.contains(ce.box('n'))).toBeUndefined();
  ...
  it('still decides membership in infinite ranges (ZZGreaterEqual shape)', () => {
    const ce = freshEngine();
    const range = ce.box(['Range', 1, 'PositiveInfinity']);
    expect(range.contains(ce.number(5))).toBe(true);
    expect(range.contains(ce.number(0))).toBe(false);
    expect(range.contains(ce.box('PositiveInfinity'))).toBe(false);
  });

  it('is undefined for symbolic bounds', () => {
    const ce = freshEngine();
    const range = ce.box(['Range', 1, 'n']);
...
describe('Union contains (Kleene OR)', () => {
  ...
  it('discharges a stored membership fact (atan/0ee626 shape)', () => {
    const ce = freshEngine();
    ce.pushScope();
    expect(ce.assume(ce.box(['Element', 'z', EXTENDED_CC]))).toBe('ok');
    expect(ce.verify(ce.box(['Element', 'z', EXTENDED_CC]))).toBe(true);
    ce.popScope();
  });
});
...
describe('Intersection contains (Kleene AND)', () => {
  it('refutes when any operand refutes, even with indeterminate operands', () => {
    const ce = freshEngine();
    const i = ce.box(['Intersection', ['Set', 'x'], ['Set', 1, 2]]).evaluate();
    // For a lazy Intersection expression: 5 is refuted by {1, 2}
    const lazy = ce.box(['Intersection', 'RealNumbers', ['Set', 1, 2]]);
    expect(lazy.contains(ce.number(5))).toBe(false);
    expect(lazy.contains(ce.number(1))).toBe(true);
    expect(lazy.contains(ce.box('y'))).toBeUndefined();
...
describe('compound-subject membership facts', () => {
  it('verifies a stored NotElement fact with a compound subject', () => {
    const ce = freshEngine();
    ce.pushScope();
    ce.assume(ce.box(['Element', 'ell', 'ComplexNumbers']));
    ce.assume(ce.box(['Element', 'eta', 'ComplexNumbers']));
    const subject = [
      'Add',
      1,
      'ell',
    ...
    expect(
      ce.assume(ce.box(['NotElement', subject, 'NonPositiveIntegers']))
    ).toBe('ok');
    expect(
      ce.verify(ce.box(['NotElement', subject, 'NonPositiveIntegers']))
    ).toBe(true);
    ce.popScope();
    expect(
      ce.verify(ce.box(['NotElement', subject, 'NonPositiveIntegers']))
...
describe('regression: literal membership unchanged', () => {
  it('number-set membership for literals', () => {
    const ce = freshEngine();
    expect(ce.box('Integers').contains(ce.number(3))).toBe(true);
    expect(ce.box('Integers').contains(ce.number(0.5))).toBe(false);
    expect(ce.box('RealNumbers').contains(ce.number(0.5))).toBe(true);
    expect(ce.box('RealNumbers').contains(ce.box('ImaginaryUnit'))).toBe(
      false
    );
    expect(ce.box('PositiveNumbers').contains(ce.number(-2))).toBe(false);
    expect(ce.box('ComplexNumbers').contains(ce.box('ImaginaryUnit'))).toBe(
      true
    );
  });
  ...
  it('finite interval membership for literals', () => {
    const ce = freshEngine();
    const closed = ce.box(['Interval', 0, 1]);
  ...
  it('Element evaluation for literals', () => {
    const ce = freshEngine();
    expect(
      ce.box(['Element', 3, ['Set', 1, 2, 3]]).evaluate().symbol
    ).toBe('True');
    expect(
      ce.box(['Element', 5, ['Set', 1, 2, 3]]).evaluate().symbol
    ).toBe('False');
    expect(
      ce.box(['Element', 3, ['Range', 1, 5]]).evaluate().symbol
    ).toBe('True');
  });

  it('SetMinus membership for literals', () => {
    const ce = freshEngine();
    const s = ce.box(['SetMinus', 'ComplexNumbers', ['Set', 0]]);
    expect(s.contains(ce.number(1))).toBe(true);
    expect(s.contains(ce.number(0))).toBe(false);
    // Symbolic: indeterminate (z could be 0)
    expect(s.contains(ce.box('z'))).toBeUndefined();
  });
});
