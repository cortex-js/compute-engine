...
describe('Primality Test', () => {
  test('Primality Test', () =>
    expect(
      ...
      eval-auto = -floor(cos((pi * ((n - 1)! + 1)) / n))
      eval-mach = -floor(cos((pi * ((n - 1)! + 1)) / n))
      N-auto    = -floor(cos((3.14159265358979323846 * ((n - 1)! + 1)) / n))
      N-mach    = -floor(cos((3.141592653589793 * ((n - 1)! + 1)) / n))
    `));
...
describe('Euler Prime Generating Polynomial', () => {
  ...
  test('x in', () =>
    expect(check('x^2 - 79x + 1601')).toMatchInlineSnapshot(`
      box       = [
        "Add",
        ["Subtract", ["Square", "x"], ["InvisibleOperator", 79, "x"]],
        1601
      ]
