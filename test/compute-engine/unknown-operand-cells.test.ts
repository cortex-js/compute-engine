...
// `u: unknown`, where the §2.2 shim produced `number`.
//
...
// widens `number ∨ integer` to `number`. An APPLICATION operand
...
// `number` came from bypassing the inference (it rebuilt facts from
...
describe('P1 — the `unknown` widening happens at boxing, not in the derivation', () => {
  ...
  test('rows 3 and 4 — the per-operator scalar claims for an inferred operand', () => {
    ...
    expect(ce2.box(['Sin', 'u3']).type.toString()).toBe('number');
  });
});
