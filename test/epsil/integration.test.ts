...
describe('EPSIL PHASE 4 — notebook integration', () => {
  test('a multi-statement program combining every v0 construct', () => {
    ...
    const { value, diagnostics } = executeEpsil(ce, program, {
  ...
  test('a one-step function definition inside a function body binds and applies', () => {
    ...
    const ce = new ComputeEngine();
    const { value } = executeEpsil(ce, 'gg(3)', {
      parseLatex: makeParseLatex(ce),
    });
...
describe('EPSIL — named inner function escapes its defining scope', () => {
