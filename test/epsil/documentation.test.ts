...
function extractEpsilBlocks(file: string): EpsilBlock[] {
  const markdown = readFileSync(file, 'utf8');
  const blocks: EpsilBlock[] = [];
  const pattern = /```(epsil(?:-live)?)\n([\s\S]*?)\n```/g;
  ...
  for (const match of markdown.matchAll(pattern)) {
    ...
    blocks.push({
      file,
      language: match[1] as EpsilBlock['language'],
      line: prefix.split('\n').length,
      source: match[2],
      expectsDiagnostics:
        previousLine.trim() === '<!-- epsil-test: expect-diagnostics -->',
...
function displayBlock(block: EpsilBlock): string {
...
describe('EPSIL DOCUMENTATION', () => {
  const blocks = DOC_FILES.flatMap(extractEpsilBlocks);

  test('frontmatter slugs are unique and Epsil links resolve', () => {
  ...
  test('all Epsil code fences parse unless marked as diagnostic examples', () => {
    const failures: string[] = [];

    for (const block of blocks) {
      const ce = new ComputeEngine();
      let diagnostics: ReturnType<typeof parseEpsil>[1];
      try {
        [, diagnostics] = parseEpsil(block.source, displayBlock(block), {
  ...
  test('live blocks and complete examples execute independently', () => {
    ...
    for (const block of executable) {
      if (block.expectsDiagnostics) continue;
      const ce = new ComputeEngine();
      const result = executeEpsil(ce, block.source, {
