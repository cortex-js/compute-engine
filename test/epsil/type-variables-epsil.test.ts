...
//
// Epsil surface of the type-variable (`forall`) feature — the D13 probes of
...
//
describe('EPSIL `forall` ANNOTATIONS (D13: full-literal positions)', () => {
  test('a `forall` annotation parses via the shared type DSL', () => {
    expect(validEpsil('let f: forall T. (T) -> T')).toStrictEqual([
      'Declare',
      'f',
      { str: 'forall T. (T) -> T' },
    ]);
  ...
  test('a bare (keyword-less) annotation takes the clause too', () => {
    expect(validEpsil('f: forall T. (T) -> T')).toStrictEqual([
      'Declare',
      'f',
      { str: 'forall T. (T) -> T' },
    ]);
  ...
  test('multiple variables, bounds, and nested signatures all parse', () => {
    expect(
      validEpsil('let f: forall T, U. (list<T>, (T) any -> U) -> list<U>')
    ).toStrictEqual([
      'Declare',
      'f',
    ...
    expect(
      validEpsil('let f: forall T: indexed_collection. (T) -> T')
    ).toStrictEqual([
      'Declare',
      'f',
  ...
  test('an effects specifier composes with the clause', () => {
    expect(validEpsil('let f: forall T. (T) random -> T')).toStrictEqual([
      'Declare',
      'f',
  ...
  test('…and the value serializes with the marker', () => {
    ...
    const ce = new ComputeEngine();
    const r = executeEpsil(ce, 'let f: forall T. (x: T) -> T = x |-> x\nf');
...
// the dot.
//
describe('EPSIL `forall` ANNOTATIONS (D13: prefix-mode dot termination)', () => {
  test('a SIGNATURE-typed bound terminates at the dot, not at the arrow', () => {
    ...
    expect(
      validEpsil('let g: forall T: (real) -> real. (T) -> boolean')
    ).toStrictEqual([
      'Declare',
      'g',
  ...
  test('the annotation terminates at `=` (initializer boundary)', () => {
    expect(
      validEpsil('let g: forall T: (real) -> real. (T) -> boolean = 5')
    ).toStrictEqual([
      'Declare',
      'g',
      { str: 'forall T: (real) -> real. (T) -> boolean' },
  ...
  test('the annotation terminates at `)` (parameter-list boundary)', () => {
    expect(validEpsil('f(x: forall T. (T) -> T) = x')).toStrictEqual([
      'DefineFunction',
      'f',
  ...
  test('a decimal-looking bound does not confuse the dot', () => {
    ...
    expect(validEpsil('let f: forall T: 5. (T) -> T')).toStrictEqual([
      'Declare',
      'f',
      { str: 'forall T: 5. (T) -> T' },
    ]);
...
//
describe('EPSIL `forall` × the `type` statement (D13: shadowing)', () => {
...
//
describe('EPSIL `forall` SERIALIZATION (D13: never decompose)', () => {
  test('a polytype declaration serializes as a full-literal annotation', () => {
    expect(
      serializeEpsil(['Declare', 'f', { str: 'forall T. (T) -> T' }])
  ...
  test('an UNGROUPED polytype return marker is the literal’s own signature', () => {
    const ast = validEpsil('f(x) -> forall T. (T) -> T = x');
    ...
    expect(serializeEpsil(ast as any)).toBe('function f<T>(x: T) -> T {x}');
  });
