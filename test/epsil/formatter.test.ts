...
// `serializeEpsil` at small margins (to force wrapping) and with
...
describe('EPSIL FORMATTER — wrapping', () => {
  test('a long list wraps one item per line, uniformly indented', () => {
    ...
    const out = serializeEpsil(list, NARROW);
  ...
  test('a nested stacked collection indents by one level (2 spaces) per depth', () => {
    ...
    const out = serializeEpsil(expr, { margin: 12, softMargin: 8 });
    const lines = out.split('\n');
  ...
  test('a list that fits on one line is not wrapped', () => {
    expect(serializeEpsil(['List', 1, 2, 3])).toBe('[1, 2, 3]');
  });
});

describe('EPSIL FORMATTER — interpolated strings', () => {
  test('a short interpolation stays inline', () => {
    expect(serializeEpsil(['String', "'value: '", ['Add', 'x', 'y']])).toBe(
      '"value: \\(x + y)"'
    );
  });
  ...
  test('a long interpolation still serializes to a single string literal', () => {
    ...
    const out = serializeEpsil(expr, NARROW);
...
describe('EPSIL FORMATTER — fancySymbols spacing', () => {
  ...
  test('infix operator uses the fancy surrounding space', () => {
    expect(serializeEpsil(['Add', 'a', 'b'], { fancySymbols: true })).toBe(
      `a${INFIX}+${INFIX}b`
    );
    expect(
      serializeEpsil(['Multiply', 'a', 'b'], { fancySymbols: true })
    ).toBe(`a${INFIX}×${INFIX}b`);
  });

  test('relational operator uses the fancy relational space + glyph', () => {
    expect(
      serializeEpsil(['LessEqual', 'a', 'b'], { fancySymbols: true })
    ).toBe(`a${REL}⩽${REL}b`);
  });

  test('separator uses the fancy thin space', () => {
    expect(serializeEpsil(['List', 1, 2], { fancySymbols: true })).toBe(
      `[1,${SEP}2]`
    );
  });

  test('fancy spacing survives wrapping at a small margin', () => {
    const out = serializeEpsil(['List', 'aa', 'bb', 'cc', 'dd', 'ee'], {
      fancySymbols: true,
      margin: 8,
      softMargin: 5,
    });
...
describe('EPSIL FORMATTER — no trailing whitespace', () => {
  ...
  test('default spacing: no line ends with whitespace when wrapped', () => {
    const out = serializeEpsil(['List', 1, 2, 3, 4, 5, 6, 7, 8], NARROW);
    expect(out).toContain('\n'); // actually wrapped
...
describe('EPSIL FORMATTER — a wrapped operator chain has no dangling operator', () => {
  ...
  test('the wrapped output re-parses', () => {
    for (const expr of [
      ...
      const out = serializeEpsil(expr, { margin: 25 });
      expect(out).toContain('\n');
      const [, diagnostics] = parseEpsil(out);
  ...
  test('a fenced list keeps its trailing separator (which re-parses)', () => {
    ...
    expect(parseEpsil(out)[1]).toEqual([]);
  });
});

describe('EPSIL FORMATTER — the `if` block form stacks conventionally', () => {
...
describe('EPSIL FORMATTER — string literals keep interior spaces', () => {
