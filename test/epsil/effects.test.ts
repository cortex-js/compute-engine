...
//
// Epsil effect annotations (Effects model, Stage 3 — `docs/EFFECTS-MODEL.md`,
// "Epsil surface").
//
...
// already accepts an effect set, so nothing in the Epsil grammar changes; they
// are pinned here so a regression is caught in the Epsil surface too.
//
...
describe('EPSIL EFFECTS — declaration form', () => {
  test('a `random` arrow in a `let` declaration', () => {
    expect(validEpsil('let f: (real) random -> real')).toStrictEqual([
      'Declare',
      'f',
  ...
  test('a `pure` arrow in a `let` declaration', () => {
    expect(validEpsil('let f: (real) pure -> real')).toStrictEqual([
      'Declare',
      'f',
      { str: '(real) pure -> real' },
    ]);
  ...
  test('a multi-label arrow in a `let` declaration', () => {
    expect(
      validEpsil('let f: (integer) random scope -> integer')
    ).toStrictEqual([
      'Declare',
      'f',
...
describe('EPSIL EFFECTS — parameter annotation', () => {
  test('an effectful callback parameter', () => {
    expect(validEpsil('g(f: (real) random -> real) = f(1)')).toStrictEqual([
      'DefineFunction',
      'g',
...
describe('EPSIL EFFECTS — definition form (block)', () => {
  test('the marker carries the FULL signature, not the return type', () => {
    expect(
      validEpsil('function roll(n) random -> integer { Random(Range(1, n)) }')
    ).toStrictEqual([
      'DefineFunction',
      'roll',
      [
  ...
  test('an annotated parameter contributes its type to the marker', () => {
    expect(
      validEpsil('function m(x: integer, y) random scope -> real { x }')
    ).toStrictEqual([
      'DefineFunction',
      'm',
      [
  ...
  test('effects-only: no arrow, wide result', () => {
    ...
    expect(
      validEpsil('function tick() scope { count = count + 1 }')
    ).toStrictEqual([
      'DefineFunction',
      'tick',
      [
  ...
  test('`pure` is a stated-empty effect set and survives the marker', () => {
    expect(validEpsil('function h(x) pure -> real { x + 1 }')).toStrictEqual([
      'DefineFunction',
      'h',
      [
        'Function',
...
describe('EPSIL EFFECTS — definition form (math)', () => {
  test('the marker carries the FULL signature', () => {
    expect(
      validEpsil('f(x) random -> integer = Random(Range(1, x))')
    ).toStrictEqual([
      'DefineFunction',
      'f',
      [
...
describe('EPSIL EFFECTS — regressions (no specifier anywhere)', () => {
  ...
  test('a typed-parameter block definition is unchanged', () => {
    expect(validEpsil('function g2(x: integer) -> real { x }')).toStrictEqual([
      'DefineFunction',
      'g2',
      [
        'Function',
  ...
  test('a PARENTHESIZED effect-bearing return type is a grouped type, not a contract (ruled 2026-08-01)', () => {
    ...
    expect(validEpsil('function mk(x) -> ((real) random -> real) { x }'))
      .toStrictEqual([
        'DefineFunction',
        'mk',
        [
  ...
  test('an EFFECT-FREE grouped return type is a grouped type too (ruled 2026-08-04)', () => {
    ...
    expect(validEpsil('f(x) -> ((integer) -> integer) = g')).toStrictEqual([
      'DefineFunction',
      'f',
  ...
  test('an UNGROUPED ground marker decomposes to its RESULT (ruled 2026-08-04)', () => {
    ...
    expect(serializeEpsil(def)).toBe('f(x) -> number = 2x');
...
describe('EPSIL EFFECTS — anonymous contract literals (option B, ruled 2026-08-01)', () => {
  ...
  test('a specifier-carrying anonymous literal round-trips via explicit Typed(…)', () => {
    ...
    const out = serializeEpsil(lit);
    expect(out).toBe('Function(Typed(x, "(x: unknown) random -> real"), x)');
    expect(validEpsil(out)).toStrictEqual([
      'Function',
      ['Typed', 'x', { str: '(x: unknown) random -> real' }],
      'x',
    ]);
  ...
  test('an EFFECT-FREE ground marker round-trips the same way (ruled 2026-08-04)', () => {
    ...
    expect(validEpsil(out)).toStrictEqual(lit);
    // The round trip preserves the arrow, not just the shape.
    expect(ce.box(validEpsil(out) as MathJsonExpression).type.toString()).toBe(
      '(unknown) -> integer'
    );
  });
  ...
  test('an effect-free ascription stays transparent', () => {
    expect(serializeEpsil(['Typed', 'x', { str: 'real' }] as MathJsonExpression)).toBe('x');
  });
...
describe('EPSIL EFFECTS — invalid specifier', () => {
  test('`pure` cannot be combined with another label', () => {
    ...
    const [expr, diagnostics] = parseEpsil(
...
describe('EPSIL EFFECTS — end to end through the engine', () => {
