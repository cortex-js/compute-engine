...
engine
  .expr(['N', engine.parse('\\int^2_0\\frac{3x}{5}dx')])
  .evaluate()
  .print();
...
(ce.parse('2x=\\sqrt{5x}').solve() as ReadonlyArray<Expression> | null)?.map(
  (x) => x.print()
);
...
console.info(
  (ce.parse('x^2 - 1 = 0').solve('x') as ReadonlyArray<Expression> | null)?.map(
    (x) => x.toString()
  )
);
...
console.log(ce.expr(['Sqrt', 2]).toNumericValue()[0]?.toString());

console.log(
  ce
    .expr(['Sqrt', ['Rational', 3, 4]])
    .toNumericValue()[0]
    ?.toString()
);
...
console.log(
  ce
    .expr(['Multiply', 2, ['Sqrt', ['Rational', 3, 5]]])
    .toNumericValue()[0]
    ?.toString()
);
...
ce.expr(['Expand', ce.parse('4x(3x+2)-5(5x-4)')])
  .simplify()
  .print();
...
const r1 = eqn.solve('x') as ReadonlyArray<Expression> | null;
console.info(r1?.map((x) => x.toString()).join(', '));

const e = ce.parse('x = \\sqrt{5}');
const r2 = (e.solve('x') as ReadonlyArray<Expression> | null)?.map((x) =>
  x.toString()
);
console.log(r2);
