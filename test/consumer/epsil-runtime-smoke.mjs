#!/usr/bin/env node
// Epsil runtime dist smoke test.
//
// The sibling `nodenext-smoke.mjs` covers `/epsil` TYPE resolution only. This
// test covers the RUNTIME: it imports the *built* production bundles (never
// `src/` via tsx) and actually executes a tiny Epsil program, asserting the
...
//      the MAIN entry (`compute-engine.js`) and handed to `executeEpsil` from
//      the `/epsil` subpath (`epsil.js`). This only works if the two bundles
...
// The built bundles the package's `exports` map points `.` and `./epsil` at.
const MAIN_BUNDLE = join(REPO_ROOT, 'dist', 'esm-min', 'compute-engine.js');
const EPSIL_BUNDLE = join(REPO_ROOT, 'dist', 'esm-min', 'epsil.js');

for (const bundle of [MAIN_BUNDLE, EPSIL_BUNDLE]) {
  if (!existsSync(bundle)) {
    console.error(
      `epsil-runtime-smoke: ${bundle} not found. Run \`npm run build production\` first.`
    );
    process.exit(1);
  }
}
...
function fail(message) {
  console.error(`epsil-runtime-smoke: FAILED — ${message}`);
  process.exit(1);
}

const { ComputeEngine } = await import(pathToFileURL(MAIN_BUNDLE).href);
const { executeEpsil } = await import(pathToFileURL(EPSIL_BUNDLE).href);

if (typeof ComputeEngine !== 'function')
  fail('ComputeEngine is not exported from the main bundle.');
if (typeof executeEpsil !== 'function')
  fail('executeEpsil is not exported from the /epsil bundle.');

// Engine from the MAIN entry, executor from the /epsil subpath.
const ce = new ComputeEngine();
const program = 'let x = 1/2\nif (x < 1) { x + 1 } else { 0 }';
const result = executeEpsil(ce, program);
...
// 2. Cross-subpath engine-class identity: the value produced by the /epsil
...
if (result.value.engine !== ce)
  fail(
    'cross-subpath engine identity broken — the /epsil bundle did not use ' +
      'the ComputeEngine instance from the main bundle (bundles are not ' +
      'sharing the engine chunk).'
  );

console.log(
  'epsil-runtime-smoke: PASSED — /epsil executes on the built bundle and ' +
