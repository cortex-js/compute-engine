...
| 3 | `(v) => v[1] + 1` | `(v: dictionary<any> \| indexed_collection<any>) -> broadcastable<number>` — the result type PROVES the engine derived the numeric flow, and never wrote it back | `(v: indexed_collection<number>) -> number` (it read `-> broadcastable<number>` until 2026-09-23, a result type computed before the refinement and kept after it) | IMPLEMENTED (Phase 3, 2026-09-05) |
