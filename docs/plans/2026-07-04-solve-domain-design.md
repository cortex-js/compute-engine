...
### Pythagorean-triple extension (2026-07-04)

The three-unknown homogeneous diagonal form `x² + y² = z²` is now recognized (a
follow-up to the first Phase-3 slice; same `tryDiophantineSolve` dispatch, which
already admits ≥ 2 integer-typed unknowns). Recognition: exactly three unknowns,
only the pure-square monomials (`'2,0,0'`, `'0,2,0'`, `'0,0,2'` — no constant,
linear, or cross term); after clearing denominators and dividing out the gcd of
the three coefficients, the absolute values must be exactly `(1, 1, 1)` with a
two-vs-one sign pattern (the equation is negated if needed so two coefficients
are `+1` and one is `−1`; the `−1` coordinate is the hypotenuse/z role, the
other two the legs).

Over fully unbounded ℤ the result is the **complete** two-family set (every
integer solution of `x² + y² = z²`, all signs, lies in one of these), in three
fresh integer parameters `t, t₁, t₂`:

- **Family A:** `leg₁ = t·(t₁² − t₂²)`, `leg₂ = 2·t·t₁·t₂`, `z = t·(t₁² + t₂²)`
- **Family B:** the `leg₁ ↔ leg₂` swap.

Legs map back to their original-order coordinates (swapped in family B); the
hypotenuse coordinate takes `t·(t₁² + t₂²)` — so `y² = x² + z²` correctly places
the hypotenuse form on `y`, and the negated `z² − x² − y² = 0` yields the same
two families.

**Deliberate declines** (stay inert, mirroring the existing decline discipline):

- **Weighted** forms (`4x² + y² = z²`) and **≥ 4-variable** sums
  (`x² + y² + z² = w²`): the general 3-parameter parametrization is provably
  **incomplete** there, so — since Solve only emits complete families — these are
  refused rather than under-reported. Counterexample for the quadruple case:
  `2² + 3² + 6² = 7²` is not generated by SymPy's 3-parameter family.
- **Inhomogeneous** three-unknown forms (`x² + y² − z² = 1`): a nonzero constant
  term is neither the homogeneous Pythagorean form nor (with three unknowns) a
  Pell form → inert (must not crash).
- **Bounded / half-bounded / mixed** domains: a bounded box is left to
  enumeration (always within its reach or correctly refused by its budget — no
  new capability lost, mirroring the bounded-linear deferral); half-bounded and
  mixed are filtered upstream as before.
