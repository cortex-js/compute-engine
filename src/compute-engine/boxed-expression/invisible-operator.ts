...
export function canonicalInvisibleOperator(
  ...
  //
  // A value-like TYPE: juxtaposition with an operand of this type is
  // multiplication (or scaling), never a silent `Tuple`. A union is
  // value-like when every arm is: `number | list<number>` — the type of a
  // coordinate read off a point-or-point-list helper — is a number or a list
  // of numbers, and `2 · PointX(P)` is scaling in both cases, exactly as the
  // `broadcastable<…>` spelling of the same idea below.
  const valueLikeType = (t: BoxedType): boolean => {
    const raw = t.type;
    if (typeof raw !== 'string' && raw.kind === 'union')
      return raw.types.every((arm) => valueLikeType(new BoxedType(arm)));
    return (
      t.isUnknown ||
      t.type === 'any' ||
      t.type === 'expression' ||
