 ...
 * EXACT value that `ce.number(x)` does not box as an exact value: an exact
 * rational with a power-of-two denominator, such as `1/2` or `-7/8` (the
 * only exact non-integers a double holds), or an exact integer past the
 * safe integers, such as `2^70`. `false` for a safe integer in any
 * representation, for a float (machine or bignum), for `NaN` and the
 * infinities, and for a non-number.
 *
 ...
 * `0.5 / 3` as a float where the list computed `1/2 ÷ 3 = 1/6` exactly.
 * `ce.number()` makes a double an exact integer only when it is a safe
 * integer (`Number.isSafeInteger`), so the exact `2^70` re-boxes as the
 * float `1.1805916207174113e+21` and is counted too. A float with an
 * integer value re-boxes as the same float, so it is never counted.
 *
 ...
 * `Number.isSafeInteger(x)` is exact, where a reduced-denominator test
 * would repeat the reduction `exactDoubleValue` already did.
 */
