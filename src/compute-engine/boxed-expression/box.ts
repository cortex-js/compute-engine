...
export function rebind(
  ce: ComputeEngine,
  expr: Expression,
  options?: { form?: FormOption; scope?: Scope }
): Expression {
 ...
 *  structural form, minus a `Function` literal's inference-written parameter
 *  annotations (see `BoxedFunction.json`). */
function serializedOperands(
  node: Expression & {
  ...
  const ops = isFunction(s) ? s.ops : node.ops;
  if (node.operator !== 'Function') return ops;
  return ops.map((x, i) =>
    i > 0 && isInferredTypedParameter(x) && isFunction(x) ? x.op1 : x
  );
}
