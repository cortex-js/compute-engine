...
export const UNIVARIATE_ROOTS: Rule[] = [
  ...
  // ax^n + b = 0
  {
    ...
    useVariations: true,
    condition: ({ _a, __b, _n }) =>
      !_a.has('_x') && !__b.has('_x') && !_n.isEqual(0),
  },
  ...
  // a * e^(bx) + c = 0
  {
    ...
    useVariations: true,
    condition: ({ __a, __c }) =>
      ((!__a.isEqual(0) && __c.div(__a).isNegative) ?? false) &&
      !__a.has('_x') &&
      !__c.has('_x'),
  },
  ...
  // a * log_b(x) + c = 0
  {
    ...
    useVariations: true,
    condition: ({ __a, __b }) => (!__a.isEqual(0) && __b.isPositive) ?? false,
  },
...
export function univariateSolve(
  ...
  const rhs = expr.op2;
  if (rhs.isEqual(0) === false) lhs = ['Subtract', lhs, rhs];
...
export const HARMONIZATION_RULES: Rule[] = [
  ...
  // a(x)/b -> a(x)
  {
    ...
    condition: ({ _a, _b }) => _a.has('_x') && _b.isEqual(0) === false,
  },
  // ab(x) -> b(x)
