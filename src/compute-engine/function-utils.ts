...
function rebindParameters(
  ...
  for (const param of params) {
    const name = functionLiteralParameterName(param);
    if (!name) continue;
    // ONLY anonymous placeholders (`_`, `_1`, `_2`, …). They are generated by
    // the shorthand/pipe desugaring, which canonicalizes the body before
    // wrapping it — the case this repair exists for — and being reserved
    // names, they can never collide with a user variable.
    //
    // A NAMED parameter is left alone: rebinding those wholesale moves
    // bindings in bodies that were already correct (measured: Rubi's
    // integration variable, curried literals keeping their annotations, and
    // the conflicting-arguments case all break).
