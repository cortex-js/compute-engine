...
function characteristicPolynomial(
  ...
  for (const [shift, coefficient] of coefficients) {
    if (coefficient.isSame(0)) continue;
    if (shift === 0) terms.push(coefficient);
    else if (shift === 1) terms.push(coefficient.mul(root).simplify());
    else terms.push(coefficient.mul(root.pow(shift)).simplify());
  }
...
function appendDistinctRoot(roots: Expression[], root: Expression): void {
  if (roots.some((other) => root.isSame(other))) return;
  roots.push(root);
}
