...
function getMachineValue(expr: Expression | undefined): number | null {
  ...
  if (typeof expr === 'object' && 'num' in expr) {
    const str = expr.num;
    if (str === 'NaN') return Number.NaN;
    if (str === 'Infinity' || expr === '+Infinity') return Infinity;
    if (str === '-Infinity') return -Infinity;
    if (str.endsWith('d') || str.endsWith('n')) {
      return new Decimal(str.slice(0, -1)).toNumber();
    }
