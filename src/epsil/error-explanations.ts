 ...
 *
 * Backticks are the ONLY code marker — not double quotes. `scripts/
 * build-error-docs.ts` renders these into MDX for https://epsil.dev/errors/,
 * where a backtick span becomes a code span and is exempted from the entity
 * escaping that `<`, `>`, `{` and `}` need everywhere else. Code left in
 * double quotes still reads on a terminal, but on the site it renders as
 * prose in straight quotes. Double quotes are kept for what they actually
 * quote: fragments of diagnostic text (`"for argument 2"`) and the tier
 * labels (`"Type error"`).
 */
...
The check runs twice by design: once statically, when the program is canonicalized (reported before anything runs), and again during evaluation, where the mismatch becomes an error value that propagates outward (see \`epsil doc runtime-error\`).`,
  ...
  'no-product-between-points': `Two points (tuples) were multiplied, and there is no implicit product between points. Multiplication of a point by a SCALAR is defined — it scales each component — and so is adding two points of the same arity, but \`(1, 2) * (3, 4)\` has no single meaning, so the engine rejects it rather than guessing.
...
Juxtaposition, \`\\cdot\` and \`\\times\` all parse to the same multiplication, so writing \`a \\times b\` between two points does not select the cross product — spell \`Cross(a, b)\` for that.`,
  ...
  'no-division-by-point': `A point (tuple) was used as a divisor. Dividing a point BY a scalar is defined — it scales each component, so \`(4, 6) / 2\` is \`(2, 3)\` — but there is no reciprocal of a point, so neither \`x / (1, 2)\` nor \`(1, 2) / (3, 4)\` has a meaning to give them.
...
No warning does not make the \`match\` total. A \`boolean\` subject that stays symbolic (an undecided comparison) is neither \`true\` nor \`false\`, and a declared name with no value (\`let u: light\` without an initializer) is not one of its constructors; both reach no case even when every alternative is covered. A final \`_\` case handles them.`,
  ...
  'symbol-expected': `A name was required at this position — after \`let\` or \`const\`, as a \`for\` loop's variable, as a function's or parameter's name — but something else was found there (\`let = 42\`).
...
A subtler cause: the word written there is one the grammar itself consumes. \`for = 3\` is not an assignment to a variable named \`for\` — the \`for\` starts a for-loop, and the loop machinery then finds no variable name. To use such a word as a name anyway, spell it verbatim, wrapped in backquote characters: "let \`for\` = 3" (see \`epsil doc reserved-word\`).`,
  ...
  'reserved-word': `A word the language reserves was used where it cannot be a plain identifier. Two cases share this code: an active keyword where an expression was expected — \`y = while\` reads as the start of a \`while\` loop, not as a value named \`while\` — and a literal word (\`true\`, \`false\`, \`Infinity\`, \`oo\`, \`NaN\`) used to NAME a binding (\`let NaN = 1\`): a literal can never be a binding name, in any position.
...
A spread is an override boundary: \`{"a" -> 1, ...d, "a" -> 2}\` is legal, and the second \`"a"\` deliberately overrides whatever the spread brought in — last wins, no diagnostic. And only literal keys are checked: a key computed at runtime cannot collide until the dictionary is actually built.`,
  ...
  'parameter-name-mismatch': `A lambda and its type annotation name the same parameter differently — \`const f: (a: number) -> number = (b) => b\`. A parameter name binds wherever it is written, so the annotation's \`a\` and the lambda's \`b\` would both claim the same slot, and the engine will not guess which one the body meant.
...
The boundary is the program (one file, one cell). Within it, a same-domain redefinition is a mistake with no possible intent. Interactively, re-running an edited definition as a SEPARATE program — a later notebook cell, the next REPL line — replaces the earlier one; that is the intended redefinition gesture, and is legal.`,
  ...
  'type-redefinition': `One program declares the same type name twice. A sum type's variant names count as names its statement declares, so a variant colliding with a later \`type\` statement reports this too.
...
The boundary is the program (one file, one cell): within it, a second declaration of a name is a mistake with no possible intent. To redefine a type interactively, re-run the edited declaration as a SEPARATE program (a later cell) — across programs, redefinition is the intended gesture and is legal. \`protocol\` declarations follow the same rule (see \`epsil doc protocol-redefinition\`).`,
  ...
  'protocol-redefinition': `One program declares the same protocol name twice — the protocol counterpart of \`epsil doc type-redefinition\`, with the same rule and the same boundary.
...
Within one program a redeclaration is a mistake; re-running an edited declaration as a separate program (a later notebook cell) replaces the earlier one and is the intended interactive gesture.`,
  ...
  'type-declaration-not-top-level': `A \`type\` statement appears inside a block or a function body. Types are engine-global — a type's name, constructor and conformances are visible to the whole session, never scoped to a block — so a nested declaration would promise a locality it cannot deliver. Declare the type at the top level of the program.
...
The mirror mistake, calling a property (\`area(b)\`), is reported as \`protocol-property-not-callable\`.`,
  ...
  'dot-call-not-a-protocol-function': `A function that is not a protocol member was called with a dot, as in \`xs.Sort()\`.
...
A program produces an error value of its own with \`RuntimeError("code")\` (or \`RuntimeError(ErrorCode("code", details))\`). Do not write \`Error("code")\` for that: a written \`Error\` is a STATIC diagnostic node and marks the expression around it as invalid, so a function whose body spells one is never defined.`,
  ...
  'capability-denied': `The program used a capability of the host — the quoted name, for example \`console\` for \`print\` and \`input\` — and the host that runs the program does not allow it. The call evaluates to this error value instead of reaching the host; nothing was printed or read.
...
A static diagnostic never suppresses evaluation: the program still runs exactly as written (errors are values — see \`epsil doc runtime-error\`), so the same mistake may be reported a second time by the run itself. The label distinguishes the tiers: "Type error"/"Static error" for the pre-run analysis, "Runtime error" for the run.`,
  ...
  'unknown-protocol': `A conformance test named a protocol that does not exist: \`Conforms(x, "Hashble")\` where no \`protocol Hashble\` was ever declared. A name that does not exist is a mistake to surface, so it is an error — never a quiet \`False\`, which would make a typo indistinguishable from a genuine non-conformance.
...
This error comes from the \`Conforms\` operator, whose protocol names ride as strings and so are only checkable when it runs. The \`is\` spelling of the same test (\`x is Hashable\`) resolves the name when the program is parsed, so a typo there is reported earlier, as a parse-time diagnostic, and never reaches this error.`,
  ...
  'polytype-comparison-unsupported': `A type comparison was given a QUANTIFIED type — a generic signature with a \`where\` clause, such as the type of a built-in like \`Sort\` — and comparing those is not supported: \`Subtype\`, the dynamic test (\`x is T\`, \`MatchesType\`), and \`Conforms\` all reject a quantified operand rather than guess.
