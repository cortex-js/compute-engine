...
export type DiagnosticCode =
  ...
  | 'variable-redeclaration' // %0 = name — a `let`/`const` of a name the SAME scope already declares: an earlier `let`/`const` of the block or program, a parameter of the enclosing function, or an index of the enclosing loop (user ruling 2026-09-05). Assign (`x = …`) to update a binding, or choose another name; a `let` in a NESTED block shadows an outer name legally. Reported by the static pass at the top level and by `canonicalBlock` inside a block; across programs (a re-run cell) a top-level `let` re-declares legally
