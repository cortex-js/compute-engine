...
`Expression` (e.g., `{ match: ce.box('x'), replace: ... }`). For simple symbol
...
For literal numbers (`BoxedNumber`), behaves identically to `isSame()` — no
tolerance is applied. Tolerance only applies to expressions that require
evaluation (e.g., `\\sin(\\pi)`).
