...
// Type-only import: like `execute-cortex.ts`, this module never statically
...
import { serializeCortex } from './serialize-cortex.js';

/** Longest Cortex snippet quoted in a `static-type-error` message. */
 ...
 *
 * Errors are values in Cortex: `Error("boom")` — or even
...
export function staticDiagnostics(
  ...
  ce.pushScope(undefined, 'cortex:static-check');
  try {
...
function canonicalizationDiagnostics(
  ...
  // `executeCortex()`); a single statement is not wrapped.
  ...
  for (const statement of statements) {
    ...
    const snippet = cortexSnippet(statement);
...
/** The text of a MathJSON string operand, or its Cortex form. */
function text(expr: MathJsonExpression): string {
  return stringValue(expr) ?? cortexSnippet(expr);
}

/** A statement (or subexpression) in Cortex source form, condensed to a
 * single line so it can be quoted in a diagnostic message. */
function cortexSnippet(expr: MathJsonExpression): string {
  let snippet: string;
  try {
    snippet = serializeCortex(expr).replaceAll(/\s+/g, ' ').trim();
  } catch {
    return '';
  }
 ...
 * the node carries no offsets (mirrors `executeCortex()`). */
