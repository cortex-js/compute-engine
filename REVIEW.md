...
- LOW stragglers A13, A16, D20, E16 fixed + regression-tested
  (`arithmetic.test.ts` A13/D20, `interval-arithmetic.test.ts` E16; A16 is a
  behavior-preserving hazard removal covered by the comparison/equal suites).
  A13 ∞-symbol·0 = NaN (free symbol still →0); A16 dropped a redundant
  `.simplify()` in `eq`'s no-free-variable branch; D20 `InterquartileRange`
  delegates to `Quartiles` (= Q3−Q1); E16 interval `mod` matches the floored
  scalar convention for negative modulus. **A15** no longer applies (resolved by
  intervening work — `simplifyNonCommutativeFunction` now threads `options`).
  **A14** deferred: forcing ascending tie-break order degrades textbook trig
  orderings (`-tanh·sech` → `-sech·tanh`) and churns snapshots — a maintainer
  direction call. (Baseline unchanged: `ascii-math` + 3 `@fixme` matrix.)
...
| ✅ A13 | LOW | `boxed-expression/boxed-symbol.ts:243` | `mul(0)` fastpath returns `Zero` even for infinite symbol values; the `Product.mul` slow path correctly returns NaN for ∞·0. **✓ verified + fixed (2026-06-10):** the fastpath now returns `NaN` when the symbol's value is known-infinite (or NaN); a *free* symbol keeps the conventional `·0 → 0` (`isInfinity === true`, not merely skipping to the slow path — which returned 0 for the symbol). Test in `arithmetic.test.ts` → "Infinite-symbol times zero (A13)". |
| ⏸️ A14 | LOW | `boxed-expression/order.ts:357-372` | Operator and string tie-breaks sort descending while the symbol branch and the doc comment say ascending — inconsistent canonical ordering. **✓ confirmed** (symbol branch `aSym > bSym ? 1 : -1` is ascending; operator/string are descending). **⏸️ DEFERRED (2026-06-10):** forcing ascending changes established canonical orderings in a debatably *worse* direction — e.g. `d/dx sech(x)` → `-(sech(x)·tanh(x))` instead of the textbook `-(tanh(x)·sech(x))` (matching the derivative-table comment), `-(cot(x)·csc(x))` instead of `-(csc(x)·cot(x))` — and churns several calculus/derivatives/logic snapshots. The right resolution (which branch to align) is a maintainer call; reverted the trial. |
| ☑️ A15 | LOW | `boxed-expression/simplify.ts:410` | `simplifyNonCommutativeFunction` drops `options` when re-simplifying operands (custom rules/costFunction ignored in that pass). **✓ no longer applies (2026-06-10):** `simplifyNonCommutativeFunction` now passes `options` to `simplifyOperands` (which threads it to `simplify(x, options)`) — resolved by intervening work. (A few `.simplify()` calls remain in the opt-in `strategy: 'fu'` block, a separate path, not this finding.) No change made. |
| ✅ A16 | LOW | `boxed-expression/compare.ts:131` | `eq()` calls `.simplify()` and is reachable from `isEqual`, which library evaluate handlers call — latent recursion risk per the project's documented hazard class. **✓ verified + fixed (2026-06-10):** the `.simplify()` was in the *no-free-variables* finite branch (`a.unknowns.length === 0 && b.unknowns.length === 0`), where `.N()` already evaluates the difference fully — so it was redundant; removed it (`a.sub(b).N()`), killing the hazard. Behavior-preserving (full comparison/equal/is suites green). (The analogous `Equal`-`eq` `.simplify()` from B14 stays — it operates on symbolic equation sides with free variables, where simplify is needed.) |
...
| ✅ D20 | LOW | `numerics/statistics.ts:273-281` | `interquartileRange` inconsistent with `quartiles` (`slice(mid+1)` vs `slice(mid)`); duplicated in the bignum version. **✓ verified + fixed (2026-06-10):** `interquartileRange`/`bigInterquartileRange` now delegate to `quartiles`/`bigQuartiles` and return `Q3 − Q1` (removes the divergent slice *and* the duplication). `IQR([1..9])`=4.5 (= 7 − 2.5). Test in `arithmetic.test.ts` → "InterquartileRange consistent with Quartiles (D20)". |
...
| ✅ E16 | LOW | `interval/elementary.ts:488-520` vs `javascript-target.ts:492-503` | Interval `mod` ([0,\|b\|) convention) doesn't enclose the compiled scalar mod (sign-of-b floored convention) for negative modulus. **✓ verified + fixed (2026-06-10):** the interval `mod` now matches the compiled floored convention — for a negative divisor (sign is determinate; a 0-straddling `b` is already `singular`) it returns the `(-period, 0]` result (`modLo − period, modHi − period`), reporting `singular` when `aVal.lo` sits on a multiple (the right-discontinuity). `mod([5],[-3])`=−1 encloses the scalar `((5%-3)+-3)%-3`=−1. Positive divisor unchanged. Test in `interval-arithmetic.test.ts` → "mod - negative modulus encloses the floored scalar (E16)". |
