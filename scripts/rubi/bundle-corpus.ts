...
//
// Scope: all of Chapter 1 (algebraic), plus the verified Chapter-4 trig
// slice. The trig files are listed EXPLICITLY rather than walked: only the
// `(a+b cos+c sin)^n` Weierstrass family is supported by the current
// inert-trig bridge (docs/rubi/RUBI.md §5, Phase R3 trig pilot); the rest of
// Chapter 4 needs the ExpandTrig / FixInert / UnifyInert argument-unification
// layer, so bundling it would only add dead compile weight. Append new files
// here as further trig families are verified.
